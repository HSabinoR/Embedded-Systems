...
void printEventLog() {
    ...
    char fileNameSD[SD_CARD_FILENAME_MAX_LENGTH+4] = "/sd/event_log.txt";

    FILE *sdCardFilePointer = fopen( fileNameSD, "r" );

    if (sdCardFilePointer == NULL) {
        uartPrint("Error opening file for reading.\r\n");
        return;
    }

    uartPrint("\r\n--- Event Log (SD Card) ---\r\n");

    while (fgets(event_buffer, sizeof(event_buffer), sdCardFilePointer)) {
        uartPrint(event_buffer);
    }

    fclose(sdCardFilePointer);
}

void saveEventToSDCard() {
    ...
    for (int i = 0; i < total_events; i++) {
        ...
        sdCardWriteFile("event_log.txt", buffer);
    }
}
